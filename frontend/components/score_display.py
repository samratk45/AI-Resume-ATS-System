    ...
    with mid:
        st.markdown(
            ...
            <div style="text-align:center; padding:2rem; background-color:{bg_color};
                ...
                <p style="color:var(--text-secondary); margin-top:0.5rem;">{interpretation}</p>
            </div>
            """,
...
def display_score_breakdown(analysis: Dict[str, Any]) -> None:
    ...
    for i, (label, key, max_score, icon) in enumerate(COMPONENTS):
        ...
        bar_color = (
            "var(--success-color)" if percentage >= 0.8
            else "var(--warning-color)" if percentage >= 0.6
        ...
        with left if i % 2 == 0 else right:
            ...
            st.markdown(
                f"""
                <div style="background-color:var(--border-light); border-radius:10px;
